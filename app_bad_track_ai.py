...
def add_points(horses, analysis_text, running_style_text, style_graph_text, pace_text, track_condition):
    ...
    # 不良馬場専用AIスコア
    # 通常の総合点・穴スコアとは別に、不良馬場だけで使う専用順位を作る。
    # 「データ分析に出ないが、前に行ける馬」を拾うための別AI。
    for h in horses:
        feature = form_features.get(h["馬番"], {})
        bad_score = 0
        bad_reasons = []
        ...
        if h["脚質"] == "逃げ":
            bad_score += 35
            bad_reasons.append("不良AI(逃げ) +35")
        elif h["脚質"] == "先行":
            bad_score += 25
            bad_reasons.append("不良AI(先行) +25")
        elif h["脚質"] == "差し":
            bad_score -= 5
            bad_reasons.append("不良AI(差し) -5")
        elif h["脚質"] == "追込":
            bad_score -= 20
            bad_reasons.append("不良AI(追込) -20")

        if feature.get("前走4角3番手以内"):
            bad_score += 25
            bad_reasons.append("不良AI(前走4角3番手以内) +25")

        if feature.get("近5走4角3番手以内2回以上"):
            bad_score += 20
            bad_reasons.append("不良AI(近5走前目2回以上) +20")

        if feature.get("前走0.5秒差以内"):
            bad_score += 12
            bad_reasons.append("不良AI(前走0.5秒差以内) +12")

        if feature.get("近3走馬券内2回以上"):
            bad_score += 10
            bad_reasons.append("不良AI(近3走馬券内2回以上) +10")

        # 人気薄でも拾うが、完全な消しすぎ馬は少しだけ抑制
        if h["人気"] is not None and h["人気"] >= 8:
            bad_score += 5
            bad_reasons.append("不良AI(人気薄妙味) +5")

        if feature.get("前走1秒以上負け"):
            bad_score -= 5
            bad_reasons.append("不良AI(前走1秒以上負け) -5")

        if feature.get("半年以上休養"):
            bad_score -= 10
            bad_reasons.append("不良AI(半年以上休養) -10")

        if feature.get("近3走二桁着順2回以上"):
            bad_score -= 10
            bad_reasons.append("不良AI(二桁着順2回以上) -10")

        # 不良軸スコア：通常能力は参考程度。前残り適性を主役にする。
        h["不良軸スコア"] = round(bad_score + h.get("最終軸スコア", 0) * 0.20 + h.get("展開指数", 0) * 0.50, 1)

        # 不良穴スコア：穴スコアも参考程度。前目要素を最優先。
        h["不良穴スコア"] = round(bad_score + h.get("穴スコア", 0) * 0.20 + h.get("展開指数", 0) * 0.50, 1)

        if track_condition == "不良":
            h["不良評価"] = bad_reasons
            if bad_reasons:
                h["加点理由"].append(" / ".join(bad_reasons))
...
def make_sanrenpuku_16_tickets(horses, track_condition):
    """
    通常馬場：従来の3連複16点ルール。
    不良馬場：完全別AI。不良軸スコア・不良穴スコアで選ぶ。
    """
    if track_condition == "不良":
        # 不良専用AI：人気や通常総合点ではなく、前残り適性を最優先。
        axis_pool = sorted(
            horses,
            key=lambda x: x.get("不良軸スコア", 0),
            reverse=True
        )
        ...
        remain_popular = axis_pool[:2]

        hole_pool = sorted(
            [h for h in horses if h not in remain_popular],
            key=lambda x: x.get("不良穴スコア", 0),
            reverse=True
        )

        selected_holes = hole_pool[:4]

        if len(selected_holes) < 4:
            return [], None

        popular_sorted = axis_pool
        hole_sorted = hole_pool
    ...
    info = {
        ...
        "mode": "不良専用AI" if track_condition == "不良" else "通常AI"
    }
...
if st.button("予想開始"):
    ...
    if not horses:
        ...
        for h in sorted(horses, key=lambda x: x["点数"], reverse=True):
            ...
            bad_track_text = ""
            if track_condition == "不良":
                bad_track_text = f"｜不良軸{h.get('不良軸スコア', 0)}点｜不良穴{h.get('不良穴スコア', 0)}点"
            ...
            if track_condition == "不良" and h.get("不良評価"):
                st.caption("不良AI評価: " + " / ".join(h["不良評価"]))
        ...
        if sanrenpuku16_info:
            st.write(f"買い目モード：{sanrenpuku16_info.get('mode', '通常AI')}")
            main_axis = sanrenpuku16_info["main_axis"]
            sub_axis = sanrenpuku16_info["sub_axis"]
            ...
            st.write(
                ...
                + (f"｜不良軸{main_axis.get('不良軸スコア', 0)}点" if track_condition == "不良" else "")
            )

            st.write(
                f"人気軸B：{sub_axis['馬番']} {sub_axis['馬名']}｜"
                f"総合{sub_axis['点数']}点｜複勝{sub_axis['複勝点']}点｜最終軸{round(sub_axis['最終軸スコア'], 1)}点"
                + (f"｜不良軸{sub_axis.get('不良軸スコア', 0)}点" if track_condition == "不良" else "")
            )

            st.write("穴馬採用4頭")
            st.write(
                " / ".join(
                    (f"{h['馬番']} {h['馬名']} (穴{h['穴スコア']} / 不良穴{h.get('不良穴スコア', 0)})" if track_condition == "不良" else f"{h['馬番']} {h['馬名']} (穴{h['穴スコア']})") for h in selected_holes
                )
            )
